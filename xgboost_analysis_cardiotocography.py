# -*- coding: utf-8 -*-
"""XGBoost_analysis_cardiotocography.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1HFNHRFgh0TJNun3ynILka3_kGQpuI4_b
...
def plot_conf_matrix(y_test, y_pred_bin,classes = 3, score = 1):
  cm = metrics.confusion_matrix(y_test, y_pred_bin)
...
"""Start XG Boost classification here"""

!pip install xgboost

#Train/ validation/  test split
#Use function split_data(dt,target,test_size = .20, random_state=10)
total_X_train, X_test, total_y_train, y_test = split_data(dt,target='NSP', test_size = 0.15)
#X_train, X_val, y_train, y_val = split_training_set(X_train, y_train)


dt_train = total_X_train.copy()
dt_train = dt_train.join(total_y_train)

X_train, X_val, y_train, y_val = split_data(dt_train , target= 'NSP', test_size=0.15)
...
xgb_model = XGBClassifier(random_state = 10, learning_rate = 0.1, n_estimators=20000)
...
#y_pred_quant = xgb_model.predict_proba(X_test)
...
y_pred_bin = xgb_model.predict(X_test)
    ...
    #Create the confusion matrics
cm = metrics.confusion_matrix(y_test, y_pred_bin)
plot_conf_matrix(y_test, y_pred_bin,classes = 3 , score = score)
